...
class _WorkforceStructurePageState extends State<WorkforceStructurePage> {
  ...
  Future<void> _handleDragReassign(
    Map<String, dynamic> sourceNode,
    Map<String, dynamic> targetNode,
  ) async {
    final sourceId = sourceNode['employee_id']?.toString() ?? '';
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: Text('$sourceName → $targetName'),
          actions: [
            TextButton(
  ...
  List<Map<String, dynamic>> _validParentOptions({
    required List<Map<String, dynamic>> allNodes,
    required Map<String, dynamic> sourceNode,
  }) {
    final sourceRole =
        (sourceNode['role_code'] ?? '').toString().toUpperCase();
    final sourceId = sourceNode['employee_id']?.toString();

    bool roleAllowed(Map<String, dynamic> candidate) {
      final targetRole =
          (candidate['role_code'] ?? '').toString().toUpperCase();

      if (sourceRole == 'WORKER' && targetRole == 'SUPERVISOR') return true;
      if (sourceRole == 'SUPERVISOR' && targetRole == 'SE') return true;
      if (sourceRole == 'SE' && targetRole == 'PM') return true;
      return false;
    }

    return allNodes.where((candidate) {
      final candidateId = candidate['employee_id']?.toString();
      if (candidateId == null || candidateId == sourceId) return false;
      return roleAllowed(candidate);
    }).toList();
  }

  bool _isValidDrop({
    required Map<String, dynamic> sourceNode,
    required Map<String, dynamic> targetNode,
  }) {
    final sourceRole =
        (sourceNode['role_code'] ?? '').toString().toUpperCase();
    final targetRole =
        (targetNode['role_code'] ?? '').toString().toUpperCase();

    if (sourceNode['employee_id'] == targetNode['employee_id']) return false;
                            ...
                            children: _tree
                                .map(
                                  (node) => _TreeNodeWidget(
                                    ...
                                    onDragReassign: _handleDragReassign,
